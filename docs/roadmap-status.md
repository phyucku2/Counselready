...
| 0.3b | **MFA (TOTP)** — enrolment, recovery codes, lost-device path | ⏳ | ⏳ | ⏳ | **Launch requirement, not optional** (CLAUDE.md §3: account takeover by an ex-partner is a life-safety risk). Deferred only because enrolment + recovery is its own portion. |
...
| 1.1b | Upload + case HTTP routes — now unblocked by ADR-0007's `current_user` gate | ⏳ | ⏳ | ⏳ | Next up. Per-case ownership checks and audit logging of case-material reads (CLAUDE.md §3) land here. |
